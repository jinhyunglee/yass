...
class maskedMFM(object):
    ...
    def calc_maskedData(self, score, mask, group):
        N, nfeature, nchannel = score.shape
        uniqueGroup = np.unique(group)
        Ngroup = uniqueGroup.size
        y = mask[:, np.newaxis, :] * score
        ySq = np.zeros([N, nfeature, nfeature, nchannel])
        scoreSq = np.zeros([N, nfeature, nfeature, nchannel])
        for n in range(nchannel):
            for p in range(nfeature):
                ySq[:, p, p, n] = y[:, p, n] * y[:, p, n]
                scoreSq[:, p, p, n] = score[:, p, n] * score[:, p, n]
                for p2 in range(p, nfeature):
                    ySq[:, p, p2, n] = ySq[:, p2, p, n] = y[
                        :, p, n] * y[:, p2, n]
                    scoreSq[:, p, p2, n] = scoreSq[
                        :, p2, p, n] = score[:, p, n] * score[:, p2, n]
        z = mask[:, np.newaxis, np.newaxis, :] * scoreSq + np.transpose(
            (1 - mask)[:, :, np.newaxis, np.newaxis] * (np.eye(nfeature)[np.newaxis, np.newaxis, :, :]), (0, 2, 3, 1))
        eta = z - ySq
        ...
        if Ngroup == N:
            ...
            weightmatrix = np.zeros([Ngroup, N])
            weightmatrix[group.ravel(), np.asarray(range(N)).ravel()] = 1

            y = y.reshape([N, int(y.size / float(N))], order='F')
            ySq = ySq.reshape([N, int(ySq.size / float(N))], order='F')
            eta = eta.reshape([N, int(eta.size / float(N))], order='F')
            sumY = np.dot(weightmatrix, y)
            sumYSq = np.dot(weightmatrix, ySq)
            sumEta = np.dot(weightmatrix, eta)
            sumY = sumY.reshape([Ngroup, nfeature, nchannel], order='F')
            sumYSq = sumYSq.reshape(
                [Ngroup, nfeature, nfeature, nchannel], order='F')
            sumEta = sumEta.reshape(
                [Ngroup, nfeature, nfeature, nchannel], order='F')
            weight = np.sum(weightmatrix, axis=1)
            groupMask = np.dot(weightmatrix, mask) / weight[:, np.newaxis]
...
class maskData:
    ...
    def calc_maskedData_mfm(self, score, mask, group):
        ...
        y_temp = y[:, :, np.newaxis, :]
        ySq = np.matmul(np.transpose(y_temp, [0, 3, 1, 2]), np.transpose(
            y_temp, [0, 3, 2, 1])).transpose((0, 2, 3, 1))

        score_temp = score[:, :, np.newaxis, :]
        scoreSq = np.matmul(np.transpose(score_temp, [0, 3, 1, 2]), np.transpose(
            score_temp, [0, 3, 2, 1])).transpose((0, 2, 3, 1))
        ...
        if Ngroup == N:
            ...
            #weightmatrix = sparse.csr_matrix((np.ones(N),(group.ravel(),np.arange(N))),[Ngroup,N])
            #weightmatrix = np.zeros([Ngroup, N])
            #weightmatrix[group.ravel(), np.asarray(range(N)).ravel()] = 1
            ...
            # sumEta = np.dot(weightmatrix, eta)
            # sumY = sumY.reshape([Ngroup, nfeature, nchannel], order='F')
            # sumYSq = sumYSq.reshape([Ngroup, nfeature, nfeature, nchannel], order='F')
            # sumEta = sumEta.reshape([Ngroup, nfeature, nfeature, nchannel], order='F')
            # weight = np.sum(weightmatrix, axis=1)
            # groupMask = np.dot(weightmatrix, mask) / weight[:, np.newaxis]
            # self.weightmatrix = weightmatrix
        else:
...
class vbPar:
    ...
    def update_local(self, maskedData):
        ...
        for k in range(Khat):
            # for n in range(nchannel):
            #    log_rho[:, k] = log_rho[:, k] + scipy.stats.multivariate_normal.logpdf(maskedData.meanY[:, :, n],
            #                                                                           self.muhat[:, k, n],
            #                                                                           self.invVhat[:, :, k, n] /
            # self.nuhat[k])
    ...
    def update_global(self, suffStat, param):
        ...
        for n in range(nchannel):
            for k in range(Khat):
                ...
                self.invVhat[:, :, k, n] = self.invVhat[:, :, k, n] + self.lambdahat[k] * np.dot(
                    self.muhat[:, np.newaxis, k, n], self.muhat[:, np.newaxis, k, n].T)
                temp = np.dot(self.muhat[:, np.newaxis, k, n], suffStat.sumY[
                              :, np.newaxis, k, n].T)
...
class suffStatistics:
    ...
    def __init__(self, *args):
        ...
        if len(args) == 2:
            ...
            self.calc_suffstat(maskedData, vbParam, Ngroup,
                               Khat, nfeature, nchannel)
        elif len(args) == 1:
        ...
        for n in range(nchannel):
            ...
            if nnoMask == 0:
                ...
                self.sumYSq1[:, :, visibleCluster, n] = np.sum(
                    rhat[:, np.newaxis, np.newaxis, visibleCluster] * unmaskedsumYSq[:, :, :, np.newaxis], axis=0)
...
class ELBO_Class:
    ...
    def calc_ELBO_Opti(self, *args):
        ...
        fterm1temp = np.squeeze(np.sum(np.matmul(np.matmul(muhat[k_ind, :, np.newaxis, :], Vhat[
                                k_ind, :, :, :]), muhat[k_ind, :, :, np.newaxis]), axis=1, keepdims=False), axis=(1, 2))
        fterm1 = -fterm1temp*vbParam.nuhat[k_ind]*suffStat.Nhat[k_ind]/2.0

        fterm2 = -2.0*np.squeeze(np.sum(np.matmul(np.matmul(sumY[k_ind, :, np.newaxis, :], Vhat[
                                 k_ind, :, :, :]), muhat[k_ind, :, :, np.newaxis]), axis=(1), keepdims=False))
        fterm2 *= -vbParam.nuhat[k_ind]/2.0

        fterm3 = np.sum(suffStat.sumYSq1[
                        :, :, k_ind, :]*vbParam.Vhat[:, :, k_ind, :], axis=(0, 1, 3), keepdims=False)
        fterm3 *= -vbParam.nuhat[k_ind]/2.0

        fterm4 = -nchannel * suffStat.Nhat[k_ind] / 2.0 * (nfeature / vbParam.lambdahat[k_ind] - nfeature * np.log(2.0) - mult_psi(vbParam.nuhat[k_ind, np.newaxis] / 2.0,
                                                                                                                                   nfeature).ravel() + nfeature * np.log(2 * np.pi))

        fterm5 = suffStat.Nhat[k_ind] * logdetVhat[k_ind]/2.0

        fterm6 = -np.sum(np.trace(np.matmul(np.transpose(suffStat.sumYSq2[:, :, k_ind, :], [2, 3, 0, 1]), Vhat[
                         k_ind, :, :, :]), axis1=2, axis2=3), axis=1, keepdims=0)*vbParam.nuhat[k_ind]/2.0
        ...
        bmterm2 = -0.5 * vbParam.nuhat[k_ind] * (
            np.sum(np.trace(Vhat[k_ind, :, :, :]/prior.V, axis1=2, axis2=3), axis=1))

        bmterm3 = 0.5 * (vbParam.nuhat[k_ind]*P + P - P*prior.lambda0/vbParam.lambdahat[k_ind] +
        ...
        # Entropy term
        entropy_term1 = np.sum(vbParam.rhat * maskedData.weight[:, np.newaxis] * (
            specsci.digamma(vbParam.ahat) - specsci.digamma(np.sum(vbParam.ahat))), axis=0)

        entropy_term2 = np.zeros(Khat)
        entropy_term2[k_ind] = -np.sum(maskedData.weight[:, np.newaxis]
                                       * rhatp*np.log(rhatp+1e-200), axis=0).ravel()
        ...
        pterm = np.log(prior.beta ** Khat *
                       np.exp(-prior.beta) / math.factorial(Khat))
...
def multivariate_normal_logpdf(x, mu, Lam):
    ...
    maha = -0.5*np.sum(np.matmul(xMinusMu, np.transpose(Lam,
                                                        [2, 0, 1])) * xMinusMu, axis=(0, 2))

    const = -0.5*p*C*np.log(2*math.pi)

    logpart = 0

    # logpart = 0.5*np.sum(np.linalg.slogdet(np.transpose(Lam,[2,0,1]))[1])
    for c in range(C):
...
def weightedKmeansplusplus(X, w, k):
    ...
    while np.unique(L).size != k:
        ...
        for i in range(1, k):
            ...
            L = np.argmax(2 * np.dot(C.T, X) - np.sum(C *
                                                      C, axis=0)[:, np.newaxis], axis=0)
        while np.any(L != L1):
            L1 = L
            for i in range(k):
                l = L == i
...
def birth_move(maskedData, vbParam, suffStat, param, L):
    ...
    Nbirth = np.sum(goodK)
    if Nbirth >= 1:
        #suffStatPrime.Nhat = suffStatPrime.Nhat[goodK]
        #suffStatPrime.sumY = suffStatPrime.sumY[:, goodK, :]
        #suffStatPrime.sumYSq = suffStatPrime.sumYSq[:, :, goodK, :]
        #suffStatPrime.sumYSq1 = suffStatPrime.sumYSq1[:, :, goodK, :]
        #suffStatPrime.sumYSq2 = suffStatPrime.sumYSq2[:, :, goodK, :]
    ...
    # if Nbirth > 1:
    #    suffStat_orig = suffStatistics(suffStat)
    #    idx_birth = Khat + np.asarray(range(Nbirth))
    #    suffStat.Nhat[idx_birth] = suffStat.Nhat[idx_birth] + suffStatPrime.Nhat
    #    suffStat.sumY[:, idx_birth, :] = suffStat.sumY[:, idx_birth, :] + suffStatPrime.sumY
    #    suffStat.sumYSq[:, :, idx_birth, :] = suffStat.sumYSq[:, :, idx_birth, :] + suffStatPrime.sumYSq
    #    suffStat.sumYSq1[:, :, idx_birth, :] = suffStat.sumYSq1[:, :, idx_birth, :] + suffStatPrime.sumYSq1
    #    suffStat.sumYSq2[:, :, idx_birth, :] = suffStat.sumYSq2[:, :, idx_birth, :] + suffStatPrime.sumYSq2
    #    vbParam.update_global_mfm(suffStat, param)
    #    suffStat = suffStatistics(suffStat_orig)
    # else:
    #    vbParam.update_global_mfm(suffStat, param)
...
def check_merge(maskedData, vbParam, suffStat, ka, kb, param, L, ELBO):
    ...
    vbParamTemp = vbPar(
        np.concatenate((vbParam.rhat[:, no_kab], np.sum(vbParam.rhat[:, [ka, kb]], axis=1, keepdims=True)), axis=1))
    suffStatTemp = suffStatistics()
    suffStatTemp.Nhat = np.append(
        suffStat.Nhat[no_kab], np.sum(suffStat.Nhat[[ka, kb]]))
    suffStatTemp.sumY = np.concatenate(
        (suffStat.sumY[:, no_kab, :], np.sum(suffStat.sumY[:, (ka, kb), :], axis=1, keepdims=True)), axis=1)
    suffStatTemp.sumYSq = np.concatenate(
        (suffStat.sumYSq[:, :, no_kab, :], np.sum(
            suffStat.sumYSq[:, :, (ka, kb), :], axis=2, keepdims=True)),
        axis=2)
    suffStatTemp.sumYSq1 = np.concatenate(
        (suffStat.sumYSq1[:, :, no_kab, :], np.sum(
            suffStat.sumYSq1[:, :, (ka, kb), :], axis=2, keepdims=True)),
        axis=2)
    suffStatTemp.sumYSq2 = np.concatenate(
        (suffStat.sumYSq2[:, :, no_kab, :], np.sum(
            suffStat.sumYSq2[:, :, (ka, kb), :], axis=2, keepdims=True)),
        axis=2)
    ...
    vbParamTemp.muhat = np.concatenate((vbParam.muhat[:, no_kab, :], suffStatTemp.sumY[
                                       :, [K-2], :] / vbParamTemp.lambdahat[K-2]), axis=1)
    nfeature, Khat, nchannel = suffStat.sumY.shape
    ...
    for n in range(nchannel):
        ...
        VhatTemp[:, :, 0, n] = np.linalg.solve(
            invVhatTemp[:, :, 0, n], np.eye(nfeature))
    ...
    ELBO_amerge = ELBO_Class(maskedData, suffStatTemp,
                             vbParamTemp, param, [K-2])
